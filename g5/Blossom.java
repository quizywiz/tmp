//Borrowed from Group 1. All credit for this code goes to group 1.
...
//    	System.out.println("Inside Blossom");
    ...
    public Blossom(int[][] edges, boolean maxcardinality) {
        ...
        List<Integer>[] blossomchilds = new ArrayList[2 * nvertex];
        ...
        List<Integer>[] blossomendps = new ArrayList[2 * nvertex];
        ...
        List<Integer> unusedblossoms = new ArrayList<Integer>();
        for (int i = 0; i < nvertex; i++) {
    ...
    private List<Integer> blossomLeaves(int b) {
    ...
    private void addBlossom(int base, int k) {
        ...
        List endps = new ArrayList<Integer>();
    ...
    private void expandBlossom(int b, boolean endstage) {
        ...
        if (!endstage && label[b] == 2) {
            ...
            while (j != 0) {
                ...
                label[endpoint[blossomendps[b].get((blossomendps[b].size() + (j - endptrick)) % blossomendps[b].size()) ^ endptrick ^ 1]] = 0;
                assignLabel(endpoint[p ^ 1], 2, p);
                // Step to the next S-sub-blossom and note its forward endpoint.
                allowedge[blossomendps[b].get((blossomendps[b].size() + (j - endptrick)) % blossomendps[b].size()) / 2] = true;
                j += jstep;
                p = blossomendps[b].get((blossomendps[b].size() + (j - endptrick)) % blossomendps[b].size()) ^ endptrick;
            ...
            j += jstep;
            while (blossomchilds[b].get((blossomchilds[b].size() + j) % blossomchilds[b].size()) != entrychild) {
                // Examine the vertices of the sub-blossom to see whether
                // it is reachable from a neighbouring S-vertex outside the
                // expanding blossom.
                bv = blossomchilds[b].get((blossomchilds[b].size() + j) % blossomchilds[b].size());
                if (label[bv] == 1) {
    ...
    private void augmentBlossom(int b, int v) {
        ...
        while (j != 0) {
            ...
            j += jstep;
            t = blossomchilds[b].get((blossomchilds[b].size() + j) % blossomchilds[b].size());
            int p = blossomendps[b].get((blossomendps[b].size() + (j - endptrick)) % blossomendps[b].size()) ^ endptrick;
            if (t >= nvertex) {
        ...
        for (int c = 0; c < i; c++) {
            blossomchilds[b].add(blossomchilds[b].get(c));
            blossomendps[b].add(blossomendps[b].get(c));
        }
        blossomchilds[b].subList(0, i).clear();
        blossomendps[b].subList(0, i).clear();
    ...
    private void verifyOptimum() {
        ...
        for (int k = 0; k < nedge; k++) {
            ...
            List<Integer> iblossoms = new ArrayList<Integer>();
            iblossoms.add(i);
            List<Integer> jblossoms = new ArrayList<Integer>();
            jblossoms.add(j);
            while (blossomparent[iblossoms.get(iblossoms.size() - 1)] != -1) {
                iblossoms.add(blossomparent[iblossoms.get(iblossoms.size() - 1)]);
            }
            while (blossomparent[jblossoms.get(jblossoms.size() - 1)] != -1) {
                jblossoms.add(blossomparent[jblossoms.get(jblossoms.size() - 1)]);
            }
            ...
            for (int c = 0; c < iblossoms.size(); c++) {
                int bi = iblossoms.get(c);
                int bj = jblossoms.get(c);
                if (bi != bj) {
                    break;
                }
                s += 2 * dualvar[bi];
